...
docker-build-base:
	DOCKER_BUILDKIT=1 docker build -f Dockerfile --target platform-base -t docintel-platform-base:3.11 .

docker-ensure-base:
	@docker image inspect docintel-platform-base:3.11 >/dev/null 2>&1 || $(MAKE) docker-build-base

docker-build: docker-ensure-base
	DOCINTEL_DOCKER_TARGET=slim $(COMPOSE) build api

docker-build-ocr: docker-ensure-base
...
docker-up-ocr:
	DOCINTEL_DOCKER_TARGET=ocr $(MAKE) docker-ensure-base
...
docker-up-full: docker-down docker-ensure-base
	DOCINTEL_DOCKER_TARGET=ocr $(COMPOSE) up -d --build redis api worker ui
	@$(MAKE) --no-print-directory up-status
	@$(MAKE) --no-print-directory up-logs-tail
