...
| OCR stack | Default Docker is `slim`. Scanned PDF OCR uses the same base (PyTorch + EasyOCR baked in). Use `make up-ocr` for the OCR app tag |
| Docker base | Run `make docker-build-base` once when `pyproject.toml` deps change. App code changes do not rebuild the base or re-download spaCy |
...
- Prometheus metrics cover HTTP traffic, background jobs, queue depth, and Redis health. Integration guide: [MONITORING.md](MONITORING.md).
